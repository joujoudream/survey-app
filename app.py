...
# 🎨 الـ CSS القوي لتوحيد التصميم والمظهر الهندسي بالكامل
ultimate_css = """
<style>
...
/* 🔵 المربع الأزرق اليمين واليسار الموحد تماماً */
...
/* زر شفاف مخفي تماماً فوق الصندوق الميداني ليعمل بالكامل عند الضغط في أي مكان داخل المربع */
...
with col2:
    ...
    # 📊 المربعات الإحصائية المتطابقة باللون الأزرق الملكي بالكامل (نسخة يمين ويسار)
    stat_col1, stat_col2 = st.columns(2)
    
    with stat_col1:
        # المربع الأيمن - المجموع الكلي للعقارات
        st.markdown(f"""
    ...
    with stat_col2:
        # المربع الأيسر المتطابق تماماً - زر عدد عقارات المنطقة المحلية
        display_name = region_input if region_input else "..."
        
        # إنشاء مظهر الصندوق الموحد بالكامل هيدروليكياً
        st.markdown(f"""
        ...
        # وضع الزر المخفي الشفاف تماماً فوق الصندوق البرمجي لنفس الوظيفة والتحكم
        if st.button("", key="hidden_blue_trigger_btn", use_container_width=True):
            st.session_state.focus_on_region = True
            st.rerun()
            
        # كود لإجبار الزر الفرعي الصغير أن يتمدد ويصبح شفافاً كلياً ويغطي المساحة المخصصة له
        st.markdown("""
        <style>
