...
with col2:
    ...
    # محرك البحث الفوري والتصحيح والتعديل (محمي تماماً الآن ضد أي KeyError)
    ...
    <div class='info-file-card'>
        ...
        <div class='info-file-text'>
            ...
            • <b>صندوق الرفع الآمن:</b> تم تحصين الصندوق العلوي بالكامل، يمكنك سحب وإفلات أي سبريدشيت مجهز وسيعتمد فوراً دون أي أخطاء صفحة بيضاء.<br>
            • <b>إدخال البيانات السريعة:</b> اكتب اسم المنطقة، واضغط <b>Enter</b> لينتقل المؤشر تلقائياً لحقل رقم العقار المالي والبدء الفوري بالعمل.<br>
            • <b>منع التكرار التلقائي:</b> يفحص البرنامج التكرار تلقائياً ويمنع إدخال نفس العقار مرتين بالخطأ في نفس المنطقة لتأمين جودة التقرير.<br>
            • <b>المزامنة السحابية الفورية:</b> كل عقار جديد تحفظه أو ملف ترفعه، يتم بث نسخه منه مباشرة لمستودع <b>GitHub</b> لضمان أمان أرشيف العمل.
        </div>
    </div>
    ...
    st.components.v1.html("".join(js_code), height=0)import streamlit as st
import pandas as pd
import requests
import base64
import os
